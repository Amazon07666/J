    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if (cookiesArr[i]) {
            ...
            for (let j = 0; j < helpUserNum; ++j) {
                ...
                if ($.todo == 3) { //寮€鍥笂闄�,鎺掗櫎涓嶅啀鍙備笌浜掑姪
                    exclude.push(j)
                    continue
                } else if ($.todo == 5) { //娲诲姩鐏垎
                    exclude.push(j)
                    continue
                }
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.success) {
                            ...
                            if (data.resultCode === '9200008') console.log('涓嶈兘鍔╁姏鑷繁')
                            else if (data.resultCode === '9200011') console.log('宸茬粡鍔╁姏杩�')
                            else if (data.resultCode === '2400205') console.log('鍥㈠凡婊�')
                            else if (data.resultCode === '2400203') {
                                console.log('鍔╁姏娆℃暟宸茶€楀敖');
                                $.canHelp = false
                            } else console.log(`鏈煡閿欒`)
                        }
                    }
                }
            } catch (e) {
            ...
            try {
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        data = JSON.parse(data);
                        if (data['success']) {
                            $.log(`褰撳墠銆愯禋浜眴(寰俊灏忕▼搴�)-鐡滃垎浜眴銆戣兘鍚﹀啀娆″紑鍥�: ${data.data.canStartNewAssist ? '鍙互' : '鍚�'}`)
                            console.log(`assistStatus ${data.data.assistStatus}`)
                            if (data.data.assistStatus === 1 && !data.data.canStartNewAssist) {
                                console.log(`宸插紑鍥�(鏈揪涓婇檺)锛屼絾鍥㈡垚鍛樹汉鏈弧\n`)
                                $.todo = 1
                                    //缁х画鍔╁姏
                            } else if (data.data.assistStatus === 3 && data.data.canStartNewAssist) {
                                console.log(`宸插紑鍥�(鏈揪涓婇檺)锛屽洟鎴愬憳浜哄凡婊n`)
                                $.todo = 2
                                    //缁х画寮€鍥�
                            } else if (data.data.assistStatus === 3 && !data.data.canStartNewAssist) {
                                console.log(`浠婃棩寮€鍥㈠凡杈句笂闄愶紝涓斿綋鍓嶅洟鎴愬憳浜哄凡婊n`)
                                $.todo = 3
                                    //鍒犻櫎浜掑姪鐮�
                            } else {
                                $.todo = 4
                            ...
                            $.todo = 5
                            $.tuan = true; //娲诲姩鐏垎
                            console.log(`璧氫含璞�(寰俊灏忕▼搴�)-鐡滃垎浜眴銆戣幏鍙栥€愭椿鍔ㄤ俊鎭け璐� ${JSON.stringify(data)}\n`)
                        }
                    }
                }
            } catch (e) {
